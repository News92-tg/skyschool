...
(function(){
  ...
  const labels={all:'Все',mate1:'Мат в 1',fork:'Вилка',pin:'Связка',skewer:'Сквозной удар',discovered:'Вскрытая атака',defence:'Защита',endgame:'Эндшпиль'};
  ...
  function patchTypeLabel(){
    ...
    window.loadPuzzle=function(i){
      ...
      setTimeout(()=>{
        ...
        const p=arr[idx];
        const type=$('#pType');
        if(type) type.textContent=labels[p.category||p.type||'all']||'Тактика';
      },0);
    };
  }
  ...
  function ensure(){
    ...
    host.innerHTML=`<div class="pf-head"><div><b>Тип задачи</b><span class="pf-count" id="pfCount"></span></div></div><div class="pf-row" id="pfCats"></div><div class="pf-row pf-diff" id="pfDiff"></div>`;
  ...
  function apply(){
    ...
    const target=window.CHESS_PUZZLES; if(!Array.isArray(target))return;
    ...
    else {const title=$('#pTitle');if(title)title.textContent='Нет задач для выбранного фильтра';}
  }
  ...
  function init(){styles();if(!ensure()){setTimeout(init,50);return;}patchTypeLabel();apply();}
