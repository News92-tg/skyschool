/* SkyySchool — universal interactive chess lessons, v2.
   One deterministic teaching system for every lesson: real position,
   legal replay, piece-aware animation, step controls, coordinates and a
...
(function(){
  ...
  const PHASES={
    l01:{ru:'Основа',en:'Basics'},l02:{ru:'Основа',en:'Basics'},l03:{ru:'Основа',en:'Basics'},
    l04:{ru:'Основа',en:'Basics'},l05:{ru:'Основа',en:'Basics'},l06:{ru:'Основа',en:'Basics'},
    l07:{ru:'Тактика',en:'Tactics'},l08:{ru:'Тактика',en:'Tactics'},l09:{ru:'Тактика',en:'Tactics'},
    l10:{ru:'Тактика',en:'Tactics'},l11:{ru:'Дебют',en:'Opening'},l12:{ru:'Дебют',en:'Opening'},
    l13:{ru:'Расчёт',en:'Calculation'},l14:{ru:'Эндшпиль',en:'Endgame'},l15:{ru:'Эндшпиль',en:'Endgame'},
    l16:{ru:'Практика',en:'Practice'}
  };
  ...
  function parseLesson(l){
    const start=E.create(l.fen||E.START_FEN), states=[snap(start)], moves=[];
    let st=start;
    for(const notation of Array.isArray(l.moves)?l.moves:[]){
      const m=E.findMove(st,notation); if(!m) break;
      const before=snap(st),san=E.toSAN(st,m); E.make(st,m);
      moves.push({move:Object.assign({},m),before,san,after:snap(st),notation}); states.push(snap(st));
    }
  ...
  function makeBoard(){
    ...
    for(let r=8;r>=1;r--)for(let f=0;f<8;f++){
      ...
      const rn=document.createElement('span'); rn.className='sli-rank'; rn.textContent=r;
      const fl=document.createElement('span'); fl.className='sli-file'; fl.textContent=FILES[f];
      if(f===0)c.appendChild(rn); if(r===1)c.appendChild(fl); b.appendChild(c);
    }
    return b;
  }
  ...
  function clearVisual(b){$$('.sli-piece,.sli-dot,.sli-last,.sli-target,.sli-check,.sli-turn,.sli-flying',b).forEach(x=>{if(x.classList?.contains('sli-last')||x.classList?.contains('sli-target')||x.classList?.contains('sli-check')||x.classList?.contains('sli-turn'))x.classList.remove('sli-last','sli-target','sli-check','sli-turn');else x.remove();});}
  function renderBoard(b,st,last){
    clearVisual(b);
    if(last){const f=E.toAlg(last.from),t=E.toAlg(last.to);b.querySelector(`[data-square="${f}"]`)?.classList.add('sli-last');b.querySelector(`[data-square="${t}"]`)?.classList.add('sli-target');}
    if(E.inCheck(st))b.querySelector(`[data-square="${E.toAlg(st.kings[st.turn])}"]`)?.classList.add('sli-check');
    for(let r=8;r>=1;r--)for(let f=0;f<8;f++){
      const sq=FILES[f]+r,p=pieceAt(st,sq),c=b.querySelector(`[data-square="${sq}"]`); if(!p||!c)continue;
      const el=document.createElement('span');el.className='sli-piece '+((p&8)?'black':'white');el.textContent=GLYPH[p&7];el.dataset.square=sq;c.appendChild(el);
    }
  }
  function rafMove(b,p,from,to,duration){
    const a=center(b,from),z=center(b,to); if(!a||!z||!p)return Promise.resolve();
    return new Promise(resolve=>{
      let start=null,raf=0,cancelled=false;
      p.style.position='absolute';p.style.left=a.x+'px';p.style.top=a.y+'px';p.style.transform='translate(-50%,-50%)';
      const ease=t=>1-Math.pow(1-t,3);
      const tick=now=>{if(cancelled)return;if(start===null)start=now;const t=Math.min(1,(now-start)/duration),e=ease(t);p.style.left=(a.x+(z.x-a.x)*e)+'px';p.style.top=(a.y+(z.y-a.y)*e)+'px';if(t<1)raf=requestAnimationFrame(tick);else{p.style.left=z.x+'px';p.style.top=z.y+'px';resolve();}};
      raf=requestAnimationFrame(tick);p.__cancel=()=>{cancelled=true;cancelAnimationFrame(raf);resolve();};
    });
  }
  function knightWay(from,to){
    const a=E.toAlg(from),b=E.toAlg(to),fa=E.fileOf(from),ra=E.rankOf(from),fb=E.fileOf(to),rb=E.rankOf(to),dx=fb-fa,dy=rb-ra;
    if(Math.abs(dx)===2&&Math.abs(dy)===1)return [FILES[fa+(dx>0?2:-2)]+(ra+1),b];
    if(Math.abs(dx)===1&&Math.abs(dy)===2)return [FILES[fa]+(ra+(dy>0?3:-1)),b];
    return [b];
  }
  function animateOne(b,info){
    const m=info.move,before=info.before,from=E.toAlg(m.from),to=E.toAlg(m.to),type=m.piece&7;
    const source=b.querySelector(`[data-square="${from}"]`),piece=source?.querySelector('.sli-piece');
    if(!source||!piece){renderBoard(b,info.after,m);return Promise.resolve();}
    ...
    const run=async()=>{for(const next of path){await rafMove(b,piece,cur,next,type===E.KNIGHT?220:320);cur=next;}renderBoard(b,info.after,m);};
    return run();
  }
  function buildCard(l,i,total){
    ...
    let idx=0,playing=false,timer=0,token=0;
    ...
    function stop(){playing=false;clearTimeout(timer);token++;const p=$('.sli-flying',board);if(p?.__cancel)p.__cancel();}
    async function go(target,animate){stop();target=Math.max(0,Math.min(data.moves.length,target));if(target===idx){paint();return;}const my=++token;if(!animate){idx=target;paint();return;}while(idx<target&&my===token){await animateOne(board,data.moves[idx]);if(my!==token)return;idx++;paint();}}
    function autoplay(){if(playing)return;if(!data.moves.length)return;if(idx>=data.moves.length)idx=0;playing=true;const my=++token;const run=async()=>{while(playing&&my===token&&idx<data.moves.length){await animateOne(board,data.moves[idx]);if(my!==token)return;idx++;paint();await new Promise(r=>setTimeout(r,380));}playing=false;};run();}
    ...
    $('.lesson-system-head',card).onclick=()=>{stop();const body=$('.lesson-system-body',card);body.classList.toggle('hidden');$('.lesson-system-head',card).setAttribute('aria-expanded',String(!body.classList.contains('hidden')));};
    ...
    if(data.error){$('.lesson-error',card).textContent='Некоторые шаги позиции не удалось разобрать — показана доступная последовательность.';}
    paint();return card;
  }
  function style(){
    if($('#sli-v2-style'))return;const s=document.createElement('style');s.id='sli-v2-style';s.textContent=`
      #tab-lessons #lessonList{display:grid!important;gap:12px!important}
      #tab-lessons .lesson-system{border:1px solid var(--line)!important;border-radius:18px!important;background:var(--panel)!important;overflow:hidden!important}
  ...
  function renderAll(){
    ...
    const cards=lessons.map((l,i)=>buildCard(l,i,lessons.length));host.replaceChildren(...cards);host.dataset.lessonSystem='v2';cards[0]?.classList.add('open');$('.lesson-system-body',cards[0])?.classList.remove('hidden');$('.lesson-system-head',cards[0])?.setAttribute('aria-expanded','true');
  }
  function watch(){
    const host=$('#lessonList');if(!host||host.__sliObserver)return;
    const obs=new MutationObserver(()=>{clearTimeout(host.__sliTimer);host.__sliTimer=setTimeout(()=>{if(host.dataset.lessonSystem!=='v2')renderAll();},20);});obs.observe(host,{childList:true,subtree:true});host.__sliObserver=obs;
  }
  ...
  boot();
  document.addEventListener('langchange',()=>setTimeout(()=>{const host=$('#lessonList');if(host)host.dataset.lessonSystem='';renderAll();},0));
})();