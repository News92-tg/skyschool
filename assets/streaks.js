...
(function () {
  ...
  /* Проверяем «дыры» за последние несколько дней и закрываем их
     ...
     страницы — чтобы серия не обнулилась молча. */
  function autoFreeze() {
    const byDay = Sky.stats().byDay || {};
    const frozen = new Set(Sky.get(FROZEN_KEY, []));
  ...
  function init() {
    ...
    const usedFreeze = autoFreeze();
    maybeAwardFreeze(computeStreak());
    renderBadge();
    /* Sky.toast рисует одно и то же всплывающее окно: если заморозка
       только что списалась, немедленное напоминание затрёт сообщение о
       ней — а оно важнее, потому что объясняет, куда делась заморозка.
       Поэтому напоминание в этом случае показываем чуть позже. */
    if (usedFreeze) setTimeout(maybeRemind, 4200); else maybeRemind();
