/* SkyySchool — единый UX-слой шахмат.
   Исправляет режимы игры и даёт нормальный конструктор собственных задач. */
'use strict';
(function(){
  ...
  function style(){
    ...
    s.textContent=`
      ...
      #tab-game .game-mode-panel{width:100%;box-sizing:border-box;overflow:visible}
      ...
      .builder-head{margin-bottom:12px}
      ...
      .builder-toolbar{display:flex;gap:5px;flex-wrap:wrap;margin-bottom:10px}
  ...
  function removeLegacyCoach(){
    ...
    $('#tab-trainer')?.remove();
    const src=$('#chessTeacherPicker')?.closest('.section');
    if(src)src.classList.add('chess-trainer-source');
  }
  ...
  function syncAnalysisOption(option){
    ...
    const on=analysisEnabled();sw.classList.toggle('is-on',on);sw.setAttribute('aria-pressed',String(on));
    const input=sw.querySelector('.analysis-check');if(input)input.checked=on;
  }
  ...
  function addAnalysisOption(panel){
    ...
    if(!option){
      ...
      option.innerHTML=`<div class="analysis-copy"><div class="analysis-title">${tx('Разбор ходов и подробная оценка','Move review and detailed game analysis')}</div><div class="analysis-desc">${tx('Показывать качество ваших ходов и итоговый разбор партии.','Show move quality and a detailed game review.')}</div></div><button type="button" class="analysis-switch"><input class="analysis-check" type="checkbox"></button>`;
      panel.insertBefore(option,panel.querySelector('.game-action-row')||null);
      option.querySelector('.analysis-switch').addEventListener('click',e=>{e.preventDefault();e.stopPropagation();localStorage.setItem(ANALYSIS_KEY,analysisEnabled()?'0':'1');syncAnalysisOption(option);const review=$('#gReview');if(review)review.classList.toggle('hidden',!analysisEnabled())});
    }
  ...
  function syncGame(){
    ...
    const solo=isSolo();game.classList.toggle('game-no-coach',solo);game.classList.toggle('game-with-coach',!solo);
  ...
  function patchNewGame(){
    const panel=$('#gameModePanel');if(!panel||panel.__workbenchNew)return;
    panel.__workbenchNew=true;
    panel.addEventListener('click',e=>{const b=e.target.closest('#gNewProxy');if(!b)return;e.preventDefault();e.stopImmediatePropagation();try{if(typeof window.newGame==='function')window.newGame();else $('#gNew')?.click()}catch(err){console.error(err)}setTimeout(syncGame,0)},true);
  }

  /* ---------- конструктор ---------- */
  function userKey(){return window.Sky?.db?.me?.()?.id||'guest'}
  function storageKey(){return TASKS_KEY+':'+userKey()}
  function readTasks(){try{return JSON.parse(localStorage.getItem(storageKey())||'[]')}catch(_){return []}}
  function writeTasks(list){localStorage.setItem(storageKey(),JSON.stringify(list.slice(0,200)))}

  function blankState(){
    const st=E.create(E.START_FEN);
    for(let r=0;r<8;r++)for(let f=0;f<8;f++)st.board[E.sq(f,r)]=0;
    st.turn=E.WHITE;st.ep=-1;st.halfmove=0;st.fullmove=1;builderCastle='-';return st;
  }

  function stateFromFen(fen){
    const check=validatePosition(fen);if(!check.ok)return null;
    const parts=String(fen).trim().split(/\s+/);builderCastle=parts[2]&&parts[2]!=='-'?parts[2]:'-';return check.state;
  }

  function fenFromState(st){
    const rows=[];
    for(let r=7;r>=0;r--){let row='',empty=0;for(let f=0;f<8;f++){const p=st.board[E.sq(f,r)]||0;if(!p){empty++;continue}if(empty){row+=empty;empty=0}const ch=['','p','n','b','r','q','k'][p&7];row+=(p&8)?ch:ch.toUpperCase()}if(empty)row+=empty;rows.push(row)}
    return rows.join('/')+' '+(st.turn===E.WHITE?'w':'b')+' '+(builderCastle||'-')+' - 0 1';
  }

  function validatePosition(fen){
    try{
      const st=E.create(fen);
      if(!st?.board||st.kings[E.WHITE]<0||st.kings[E.BLACK]<0)return {ok:false,msg:tx('На доске должны быть оба короля.','Both kings must be on the board.')};
    ...
    }catch(_){return {ok:false,msg:tx('FEN не удалось прочитать.','The FEN could not be read.')}}
  }

  function renderBoard(){
    const host=$('#builderBoard');if(!host||!builderState)return;
    host.innerHTML='<div class="builder-board-grid">'+Array.from({length:64},(_,i)=>{const r=7-Math.floor(i/8),f=i%8,sq=E.sq(f,r),p=builderState.board[sq]||0;const light=(f+r)%2===0;const pc=p?(p&8?' black-piece':' white-piece'):'';return `<button type="button" class="builder-cell ${light?'light':'dark'}${pc}" data-sq="${sq}">${p?['','♟','♞','♝','♜','♛','♚'][p&7]:''}</button>`}).join('')+'</div>';
    $$('.builder-cell',host).forEach(c=>c.addEventListener('click',()=>{builderState.board[+c.dataset.sq]=builderPiece;renderBoard();updateFen();}));
  }

  function updateFen(){const input=$('#builderFen');if(input)input.value=fenFromState(builderState)}

  function syncPieceButtons(){
    const sec=$('#tab-builder');if(!sec)return;
    $$('.builder-piece',sec).forEach(b=>b.classList.toggle('active',+b.dataset.piece===builderPiece));
  }

  function syncCastle(){
    $$('.builder-castle input').forEach(c=>c.checked=builderCastle.includes(c.value));
  }

  function readCastle(){
    let c='';$$('.builder-castle input').forEach(x=>{if(x.checked)c+=x.value});builderCastle=c||'-';updateFen();
  }

  function showBuilderMessage(text,ok){const m=$('#builderMessage');if(!m)return;m.className='builder-message verdict '+(ok?'ok':'no');m.textContent=text}

  function builderTemplate(){
    const logged=!!window.Sky?.db?.me?.();
    return `<div class="builder-shell"><section class="builder-card"><div class="builder-head"><h2 class="builder-title">${tx('Конструктор позиции','Position builder')}</h2><p class="builder-sub">${tx('Расставьте фигуры как угодно, задайте сторону хода, решение и сохраните свою задачу.','Place any pieces, choose the side to move, set the solution and save your own puzzle.')}</p></div><div class="builder-toolbar">${PIECES.map(([id,icon])=>`<button type="button" class="builder-piece" data-piece="${id}">${icon}</button>`).join('')}<button type="button" class="builder-piece eraser" data-piece="0">⌫</button></div><div id="builderBoard" class="builder-board"></div></section><aside class="builder-card"><div class="builder-options"><div class="builder-field"><label>${tx('Сторона хода','Side to move')}</label><select id="builderTurn"><option value="w">${tx('Белые','White')}</option><option value="b">${tx('Чёрные','Black')}</option></select></div><div class="builder-field"><label>${tx('Рокировка','Castling')}</label><div class="builder-castle"><label class="builder-check"><input type="checkbox" value="K">K</label><label class="builder-check"><input type="checkbox" value="Q">Q</label><label class="builder-check"><input type="checkbox" value="k">k</label><label class="builder-check"><input type="checkbox" value="q">q</label></div></div><div class="builder-field"><label>${tx('Название','Title')}</label><input id="builderTitle" maxlength="80" type="text" placeholder="${tx('Например: Связка ладьи','For example: Rook pin')}"></div><div class="builder-field"><label>${tx('Категория','Category')}</label><select id="builderCategory"><option value="mate1">mate1</option><option value="mate2">mate2</option><option value="fork">fork</option><option value="pin">pin</option><option value="skewer">skewer</option><option value="discovered">discovered</option><option value="defence">defence</option><option value="endgame">endgame</option><option value="opening">opening</option><option value="general" selected>general</option></select></div><div class="builder-field"><label>${tx('Сложность','Difficulty')}</label><select id="builderDifficulty"><option value="easy">easy</option><option value="medium">medium</option><option value="hard">hard</option></select></div><div class="builder-field"><label>${tx('Решение (SAN)','Solution (SAN)')}</label><input id="builderSolution" type="text" placeholder="Ra8# or Ra8#, Qh7#"></div><div class="builder-field"><label>${tx('Идея / пояснение','Idea / note')}</label><textarea id="builderIdea" rows="4"></textarea></div><div class="builder-field"><label>FEN</label><input id="builderFen" class="builder-fen" readonly></div><div class="builder-actions"><button type="button" class="btn chess" id="builderSave">${tx('Сохранить задачу','Save puzzle')}</button><button type="button" class="btn ghost" id="builderReset">${tx('Новая позиция','New position')}</button></div><div id="builderMessage" class="builder-message"></div><div class="builder-cloud"><b>${tx('Сохранение','Storage')}</b>${tx('Без входа задача хранится локально. После входа через Google + Supabase задачи привязываются к вашему профилю и могут синхронизироваться между устройствами.','Without an account, puzzles stay local. After Google + Supabase sign-in, puzzles are tied to your profile and can sync across devices.')}</div>${logged?'':'<div class="builder-login"><span>'+tx('Войдите, чтобы синхронизировать свои задачи.','Sign in to sync your puzzles.')+'</span><button type="button" class="btn ghost small" id="builderLogin">'+tx('Войти','Sign in')+'</button></div>'}<div class="builder-field"><label>${tx('Мои задачи','My puzzles')}</label><div id="builderSaved" class="builder-saved"></div></div></div></aside></div>`;
  }

  function renderBuilder(){
    const sec=$('#tab-builder');if(!sec||!builderState)return;
    const old={title:$('#builderTitle')?.value,category:$('#builderCategory')?.value,difficulty:$('#builderDifficulty')?.value,solution:$('#builderSolution')?.value,idea:$('#builderIdea')?.value,turn:$('#builderTurn')?.value};
    sec.innerHTML=builderTemplate();
    renderBoard();
    $$('.builder-piece',sec).forEach(b=>b.addEventListener('click',()=>{builderPiece=+b.dataset.piece;syncPieceButtons()}));
    $('#builderTurn').value=old.turn||(builderState.turn===E.BLACK?'b':'w');
    $('#builderTurn').addEventListener('change',e=>{builderState.turn=e.target.value==='b'?E.BLACK:E.WHITE;updateFen()});
    syncCastle();
    $$('.builder-castle input').forEach(x=>x.addEventListener('change',readCastle));
    if(old.title!=null)$('#builderTitle').value=old.title;
    if(old.category)$('#builderCategory').value=old.category;
    if(old.difficulty)$('#builderDifficulty').value=old.difficulty;
    if(old.solution!=null)$('#builderSolution').value=old.solution;
    if(old.idea!=null)$('#builderIdea').value=old.idea;
    ...
    $('#builderLogin')?.addEventListener('click',()=>window.Sky?.auth?.openAuth?.());
    syncPieceButtons();updateFen();renderSaved();
  }

  function saveBuilderPuzzle(){
    const fen=fenFromState(builderState);const valid=validatePosition(fen);if(!valid.ok){showBuilderMessage(valid.msg,false);return}
    const raw=$('#builderSolution').value.trim();const sols=raw.split(',').map(x=>x.trim()).filter(Boolean);if(!sols.length){showBuilderMessage(tx('Укажите хотя бы один ход решения.','Enter at least one solution move.'),false);return}
    const legal=[];
    for(const san of sols){if(!E.findMove(valid.state,san)){showBuilderMessage(tx('Нелегальный ход в решении: ','Illegal solution move: ')+san,false);return}legal.push(san)}
    const me=window.Sky?.db?.me?.();const id=builderEditingId||`custom-${Date.now().toString(36)}-${Math.random().toString(36).slice(2,6)}`;const task={id,owner_id:me?.id||null,title:$('#builderTitle').value.trim()||tx('Моя шахматная задача','My chess puzzle'),category:$('#builderCategory').value,difficulty:$('#builderDifficulty').value,fen,solutions:legal,idea:$('#builderIdea').value.trim(),created_at:new Date().toISOString()};
    const list=readTasks();const i=list.findIndex(x=>x.id===id);if(i>=0)list[i]=task;else list.unshift(task);writeTasks(list);builderEditingId=id;renderSaved();showBuilderMessage(me&&Sky.db.isCloud?.()?tx('Задача сохранена и поставлена на синхронизацию.','Puzzle saved and queued for sync.'):tx('Задача сохранена на этом устройстве.','Puzzle saved on this device.'),true);syncCloudTask(task).catch(()=>{});
  }

  async function syncCloudTask(task){
    const db=window.Sky?.db;const me=db?.me?.();if(!db?.isCloud?.()||!me)return;
    const row={id:task.id,user_id:me.id,title:task.title,category:task.category,difficulty:task.difficulty,fen:task.fen,solutions:task.solutions,idea:task.idea,created_at:task.created_at,updated_at:new Date().toISOString()};
    const found=await db.list('chess_custom_tasks',{id:task.id});if(found.length)await db.update('chess_custom_tasks',task.id,row);else await db.insert('chess_custom_tasks',row);
  }

  async function hydrateCloud(){
    const db=window.Sky?.db;const me=db?.me?.();if(!db?.isCloud?.()||!me)return;
    try{
      const guestKey=TASKS_KEY+':guest';const guest=(()=>{try{return JSON.parse(localStorage.getItem(guestKey)||'[]')}catch(_){return []}})();
      let cloud=await db.list('chess_custom_tasks',{user_id:me.id});
      for(const task of guest){const moved={...task,owner_id:me.id};await syncCloudTask(moved);}
      if(guest.length)localStorage.removeItem(guestKey);
      cloud=await db.list('chess_custom_tasks',{user_id:me.id});
      const map=new Map(readTasks().map(x=>[x.id,x]));
      cloud.forEach(x=>map.set(x.id,{id:x.id,owner_id:x.user_id,title:x.title,category:x.category,difficulty:x.difficulty,fen:x.fen,solutions:x.solutions||[],idea:x.idea||'',created_at:x.created_at}));
      writeTasks(Array.from(map.values()).sort((a,b)=>new Date(b.created_at)-new Date(a.created_at)));renderSaved();
    }catch(_){ }
  }

  function renderSaved(){
    const host=$('#builderSaved');if(!host)return;const list=readTasks();
    if(!list.length){host.innerHTML=`<div class="builder-empty">${tx('Пока нет сохранённых задач.','No saved puzzles yet.')}</div>`;return}
    host.innerHTML='';list.forEach(task=>{const item=document.createElement('div');item.className='builder-saved-item';item.innerHTML=`<div class="builder-saved-copy"><b>${esc(task.title)}</b><span>${esc(task.category)} · ${esc(task.difficulty)} · ${esc(task.solutions?.join(', ')||'')}</span></div><button type="button" class="btn ghost small" data-open="${esc(task.id)}">${tx('Открыть','Open')}</button><button type="button" class="btn ghost small" data-delete="${esc(task.id)}">×</button>`;item.querySelector('[data-open]').addEventListener('click',()=>openSaved(task));item.querySelector('[data-delete]').addEventListener('click',async()=>{writeTasks(readTasks().filter(x=>x.id!==task.id));renderSaved();if(window.Sky?.db?.isCloud?.())await window.Sky.db.remove('chess_custom_tasks',task.id)});host.appendChild(item)})
  }

  function openSaved(task){
    const state=stateFromFen(task.fen);if(!state)return;builderState=state;builderEditingId=task.id;builderPiece=1;builderCastle=(String(task.fen).split(/\s+/)[2]||'-');renderBuilder();setTimeout(()=>{if($('#builderTitle'))$('#builderTitle').value=task.title||'';if($('#builderCategory'))$('#builderCategory').value=task.category||'general';if($('#builderDifficulty'))$('#builderDifficulty').value=task.difficulty||'easy';if($('#builderSolution'))$('#builderSolution').value=(task.solutions||[]).join(', ');if($('#builderIdea'))$('#builderIdea').value=task.idea||'';syncCastle();updateFen()},0)}

  function ensureBuilderTab(){
    const tabs=$('.tabs');if(!tabs)return;
    let b=tabs.querySelector('[data-tab="builder"]');if(!b){b=document.createElement('button');b.type='button';b.className='chess-workbench-tab';b.dataset.tab='builder';b.setAttribute('aria-selected','false');b.textContent=tx('Конструктор','Builder');tabs.appendChild(b)}
    let sec=$('#tab-builder');if(!sec){sec=document.createElement('section');sec.id='tab-builder';sec.className='hidden';$('#tab-live')?.after(sec)}
    if(!builderState)builderState=blankState();
    if(!sec.querySelector('.builder-shell'))renderBuilder();
  }

  function bindBuilderTab(){
    const tabs=$('.tabs');if(!tabs||tabs.__builderBound)return;tabs.__builderBound=true;
  ...
  function init(){
    style();removeLegacyCoach();syncGame();patchNewGame();ensureBuilderTab();bindBuilderTab();hydrateCloud();
    const game=$('#tab-game');if(game&&!game.__workbenchObserver){const mo=new MutationObserver(()=>{removeLegacyCoach();syncGame();patchNewGame()});mo.observe(game,{childList:true,subtree:true});game.__workbenchObserver=true}
    window.addEventListener('authchange',()=>setTimeout(()=>{ensureBuilderTab();hydrateCloud();renderBuilder()},0));
    document.addEventListener('langchange',()=>setTimeout(()=>{removeLegacyCoach();syncGame();ensureBuilderTab()},0));
  }
