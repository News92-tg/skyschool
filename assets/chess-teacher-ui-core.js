...
window.ChessTeacherUI = (function () {
  ...
  function installAdaptiveHooks(){
    ...
    if(review && !review.__skyAdaptiveWrapped){const originalReview=review.review;review.review=function(){const result=originalReview.apply(this,arguments);const game=document.querySelector('#tab-game');if(result&&game&&!game.classList.contains('hidden')){writeAdaptiveResult(result.quality);setTimeout(renderDifficultyUI,0);}return result;};AI.__skyAdaptiveWrapped=true;}
  }
