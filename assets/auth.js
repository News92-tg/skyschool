...
Sky.auth = (function () {
  ...
  Sky.extendDict({
    ...
    checkMail:{ru:'Профиль создан. Подтвердите почту, потом войдите.',
                en:'Profile created. Confirm your email, then sign in.'},
    ...
    googleSoon:{ru:'Google-вход появится после подключения Supabase.',
                 en:'Google sign-in will be available after Supabase is connected.'}
  });
    ...
    Sky.modal('', (box, close) => {
      const draw = () => {
        box.innerHTML = `
          ...
          <div class="field" style="margin-top:16px;margin-bottom:12px${signup ? '' : ';display:none'}">
          ...
          <button class="btn ghost full" id="aGoogle" style="margin-top:9px;gap:9px">
        ...
        // TODO: подключить после настройки Supabase Auth + Google OAuth
        box.querySelector('#aGoogle').addEventListener('click', () => {
          Sky.toast(Sky.t('googleSoon'));
        });
  ...
  /* При смене входа перерисовываем всю шапку */
