...
(async () => {
  ...
  for (const theme of ['light', 'dark']) {
    const vals = await tp.evaluate(t => {
      ...
      return {
        ...
        accentInkOnAccent: probe(pick('--accent-ink'), pick('--accent')),
        okOnSoft: probe(pick('--ok'), pick('--ok-soft')),
        warnOnSoft: probe(pick('--warn'), pick('--warn-soft'))
      };
    }, theme);
  ...
  const focus = await kb.evaluate(() => {
    ...
    return {
      ...
      reducedMotion: [...document.styleSheets].flatMap(s => { try { return [...s.cssRules]; } catch (e) { return []; } })
